...
#ifndef SimpleInstrumentBasicSynth_h
#define SimpleInstrumentBasicSynth_h
...
class SimpleInstrumentBasicSynth : public Synth{

public:
	SimpleInstrumentBasicSynth(){
		...
		*/
		...
		//TriangleWave tone = TriangleWave();
		...
		Generator oscWithEnvelope = tone * tremeloSine * ADSR().attack(0.01).decay(1.5).sustain(0).release(0).trigger(envelopeTrigger).legato(false); //good
		//Generator oscWithEnvelope = tone * tremeloSine * ADSR().attack(0).decay(0.1).sustain(0).release(0).trigger(envelopeTrigger).legato(false);
		//Generator oscWithEnvelope = tone * ADSR().attack(0.01).decay(1.5).sustain(0).release(0).trigger(envelopeTrigger).legato(false); 
...
#pragma message("C++11 Support not available - SimpleInstrumentBasicSynth disabled")

#endif
	}
};

TONIC_REGISTER_SYNTH(SimpleInstrumentBasicSynth)
